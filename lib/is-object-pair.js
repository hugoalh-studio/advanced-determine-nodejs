 ...
 * @param {boolean} [$fastForward=false]
 * @returns {(boolean|null)} Determine result.
 */
function isObjectPair(item, option = {}, $fastForward = false) {
	if (typeof $fastForward !== "boolean") {
		throw new TypeError(`Argument \`$fastForward\` must be type of boolean!`);
	};
	if ($fastForward === true) {
		return isObjectPairInternal(item);
	};
