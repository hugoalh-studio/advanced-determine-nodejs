...
function isString(item, option = {}) {
	...
	if (!isRegularExpression(option.pattern, { exactly: true, global: true }) && typeof option.pattern !== "undefined") {
	...
	if (
		(option.empty === false && item.length === 0) ||
		(option.empty === true && item.length > 0)
	) {
		return false;
	};
	return true;
