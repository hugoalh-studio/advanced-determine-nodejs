...
function isString(item, option = {}) {
	...
	option.maximumLength ??= option.maxLength ?? option.maximumCharacters ?? option.maxCharacters ?? Infinity;
	if (option.maximumLength !== Infinity && !isNumber(option.maximumLength, {
		integer: true,
		positive: true,
		safe: true
	})) {
		throw new TypeError(`Argument \`option.maximumLength\` must be \`Infinity\` or type of number (integer, positive, and safe)!`);
	}
	option.minimumLength ??= option.minLength ?? option.minimumCharacters ?? option.minCharacters ?? 0;
	if (!isNumber(option.minimumLength, {
		integer: true,
		maximum: option.maximumLength,
		positive: true,
		safe: true
	})) {
		throw new TypeError(`Argument \`option.minimumLength\` must be type of number (integer, positive, and safe) and less than or equal to argument \`option.maximumLength\`'s value!`);
	}
	option.multipleLine ??= option.multiLine ?? option.multiline;
