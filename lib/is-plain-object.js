...
function isPlainObject(item, option = {}) {
	...
	option.maximumEntries ??= option.maxEntries ?? Infinity;
	if (option.maximumEntries !== Infinity && !isNumber(option.maximumEntries, {
		integer: true,
		positive: true,
		safe: true
	})) {
		throw new TypeError(`Argument \`option.maximumEntries\` must be \`Infinity\` or type of number (integer, positive, and safe)!`);
	}
	option.minimumEntries ??= option.minEntries ?? 0;
	if (!isNumber(option.minimumEntries, {
		integer: true,
		maximum: option.maximumEntries,
		positive: true,
		safe: true
	})) {
		throw new TypeError(`Argument \`option.minimumEntries\` must be type of number (integer, positive, and safe) and less than or equal to argument \`option.maximumEntries\`'s value!`);
	}
	option.super ??= false;
	...
	return $isPlainObject(item, {
		configurableEntries: option.configurableEntries,
		enumerableEntries: option.enumerableEntries,
		getterEntries: option.getterEntries,
		maximumEntries: option.maximumEntries,
		minimumEntries: option.minimumEntries,
		setterEntries: option.setterEntries,
		symbolKeys: option.symbolKeys,
		writableEntries: option.writableEntries
	});
}
