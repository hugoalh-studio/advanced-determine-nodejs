...
function isMap(item, option = {}) {
	...
	option.maximumSize ??= option.maxSize ?? option.maximumEntries ?? option.maxEntries ?? Infinity;
	if (option.maximumSize !== Infinity && !isNumber(option.maximumSize, {
		integer: true,
		positive: true,
		safe: true
	})) {
		throw new TypeError(`Argument \`option.maximumSize\` must be \`Infinity\` or type of number (integer, positive, and safe)!`);
	}
	option.minimumSize ??= option.minSize ?? option.minimumEntries ?? option.minEntries ?? 0;
	if (!isNumber(option.minimumSize, {
		integer: true,
		maximum: option.maximumSize,
		positive: true,
		safe: true
	})) {
		throw new TypeError(`Argument \`option.minimumSize\` must be type of number (integer, positive, and safe) and less than or equal to argument \`option.maximumSize\`'s value!`);
	}
