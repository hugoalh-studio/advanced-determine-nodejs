...
function isPlainObject(item, option = {}) {
	...
	if (option.super) {
		option.containGetters = false;
		option.containSetters = false;
		option.elementsConfigurable = true;
		option.elementsEnumerable = true;
		option.elementsWritable = true;
		option.keysSymbols = false;
	};
	if (!$isPlainObject(item, { configurable: option.elementsConfigurable, empty: option.empty, enumerable: option.elementsEnumerable, get: option.containGetters, set: option.containSetters, symbol: option.keysSymbols, writable: option.elementsWritable })) {
		return false;
	};
	if (
		option.strictKeys ||
		!option.allowKeysAsterisks ||
		!option.allowKeysHyphens ||
		!option.allowKeysWhitespaces
	) {
