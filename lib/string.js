	...
	constructor({
		...
		if (typeof length === "number") {
			if (!(Number.isSafeInteger(length) && length >= 0)) {
				throw new RangeError(`Argument \`length\` must be a number which is integer, positive, and safe!`);
			}
		} else if (typeof length !== "undefined") {
			throw new TypeError(`Argument \`length\` must be type of number or undefined!`);
		}
		if (typeof lengthMaximum !== "number") {
			throw new TypeError(`Argument \`lengthMaximum\` must be type of number!`);
		}
		if (lengthMaximum !== Infinity && !(Number.isSafeInteger(lengthMaximum) && lengthMaximum >= 0)) {
			throw new RangeError(`Argument \`lengthMaximum\` must be \`Infinity\`, or a number which is integer, positive, and safe!`);
		}
		if (typeof lengthMinimum !== "number") {
			throw new TypeError(`Argument \`lengthMinimum\` must be type of number!`);
		}
		if (!(Number.isSafeInteger(lengthMinimum) && lengthMinimum >= 0 && lengthMinimum <= lengthMaximum)) {
			throw new RangeError(`Argument \`lengthMinimum\` must be a number which is integer, positive, safe, and <= ${lengthMaximum}!`);
		}
		...
		if (!(pattern instanceof RegExp) && typeof pattern !== "undefined") {
			throw new TypeError(`Argument \`pattern\` must be instance of regular expression, or type of undefined!`);
		}
