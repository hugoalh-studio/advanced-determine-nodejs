...
/**
 * @function isObjectPairStrict
 * @alias isDictionaryStrict
 * @alias isDictS
 * @alias isObjectPlainStrict
 * @alias isObjPairS
 * @alias isObjPlainS
 * @description Determine item is type of object pair (strict) or not.
 * @param {*} item Item that need to determine.
 * @param {object} [option={}] Option.
 * @param {string} [option.typeOfValue] Determine value type.
 * @returns {(boolean|null)} Determine result.
 */
function isObjectPairStrict(item, option = {}) {
	checkOption(option);
	...
	for (let element in item) {
		if (item.hasOwnProperty(element) === false) {
			return false;
		};
	};
	if (
...
module.exports = isObjectPairStrict;
