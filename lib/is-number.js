...
function isNumber(item, option = {}) {
	...
	if (option.minimum !== -Infinity && !isNumber(option.minimum, { finite: true, safe: true })) {
		throw new TypeError(`Argument \`option.minimum\` must be \`-Infinity\` or type of number (finite and safe)!`);
	};
	if (option.minimum > option.maximum) {
		throw new RangeError(`Illogical condition, argument \`option.minimum\`'s value must be less than or equal to argument \`option.maximum\`'s value!`);
	};
