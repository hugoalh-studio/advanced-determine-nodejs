import isPlainObjectInno from "./internal/is-plain-object-inno.js";
...
function isNumber(item, option = {}) {
	...
	if (typeof option.even !== "boolean" && typeof option.even !== "undefined") {
		throw new TypeError(`Argument \`option.even\` must be type of boolean or undefined!`);
	}
	option.exclusiveMaximum ??= option.exclusiveMax ?? false;
	if (typeof option.exclusiveMaximum !== "boolean") {
		throw new TypeError(`Argument \`option.exclusiveMaximum\` must be type of boolean!`);
	}
	option.exclusiveMinimum ??= option.exclusiveMin ?? false;
	...
	option.maximum ??= option.max ?? Infinity;
	if (option.maximum !== Infinity && !isNumber(option.maximum, { safe: true })) {
		throw new TypeError(`Argument \`option.maximum\` must be \`Infinity\` or type of number (safe)!`);
	}
	option.minimum ??= option.min ?? -Infinity;
	if (option.minimum !== -Infinity && !isNumber(option.minimum, {
		maximum: option.maximum,
		safe: true
	})) {
		throw new TypeError(`Argument \`option.minimum\` must be \`-Infinity\`, or type of number (safe) and less than or equal to argument \`option.maximum\`'s value!`);
	}
	option.negative ??= option.ngt ?? option.nega;
	...
	if (typeof option.odd !== "boolean" && typeof option.odd !== "undefined") {
		throw new TypeError(`Argument \`option.odd\` must be type of boolean or undefined!`);
	}
	option.positive ??= option.pst ?? option.posi;
	...
	if (
		typeof item !== "number" ||
		Number.isNaN(item) ||
		(option.even === false && itemIsSafeInteger && item % 2 === 0) ||
		(option.even === true && (
			!itemIsSafeInteger ||
			item % 2 !== 0
		)) ||
		...
		(option.odd === false && itemIsSafeInteger && item % 2 !== 0) ||
		(option.odd === true && (
			!itemIsSafeInteger ||
			item % 2 === 0
		)) ||
