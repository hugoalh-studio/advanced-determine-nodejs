...
const event = require("./internal/event.js");
 ...
 * @param {boolean} [param1.allowEmpty] Whether to allow an empty set.
...
function isSet(item, {
	...
	if (allowEmpty) {
		if (minimumSize === 1) {
			...
			event.warnConflictedArguments("minimumSize", "allowEmpty");
		}
	}
	if (
		!(item instanceof Set) ||
