...
function areEqual(...items) {
	...
	if (item1TypeOf === "map") {
		...
		for (let index = 0; index < item2Data.length; index++) {
			let key = item2Data[index][0];
			if (item1.has(key) === false) {
				return false;
			};
			if (areEqual(item2Data[index][1], item1.get(key)) === false) {
				return false;
			};
		};
		return true;
	...
	if (item1TypeOf === "set") {
		...
		for (let index = 0; index < item2Data.length; index++) {
			if (item1.has(item2Data[index]) === false) {
				return false;
			};
		};
		return true;
