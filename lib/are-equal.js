...
function areEqualInternal(item1, item2) {
	...
	if (item1TypeOf === "set") {
		...
		let item1ValuesLength = item1Values.length;
		for (let indexItem1Values = 0; indexItem1Values < item1ValuesLength; indexItem1Values++) {
	...
	let item2IsObjectPair = isObjectPairInternal(item2);
	if (item1IsObjectPair !== false && item2IsObjectPair !== false) {
		...
		if (item1KeysLength !== item2Keys.length) {
			return false;
		};
		for (let index = 0; index < item1KeysLength; index++) {
