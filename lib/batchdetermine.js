/*==================
Advanced Determine - Batch Determine
	Language:
		NodeJS/14.15.0
==================*/
const adCore = require("./core.js"),
...
function batchInternal(condition, ...items) {
	...
	if (isString(condition) === true) {
		if (condition.search(/[^a-z]/giu) !== -1) {
			throw new SyntaxError(`Argument "typeDeterminerName" contains non-letter character(s)! (Advanced Determine - Batch Determine)`);
		};
		typeDeterminerName = condition;
	} else if (isArray(condition) === true) {
		if (isString(condition[0]) !== true) {
			throw new TypeError(`Argument "typeDeterminerName" must be type of string (non-nullable)! (Advanced Determine - Batch Determine)`);
		};
		if (condition[0].search(/[^a-z]/giu) !== -1) {
			throw new SyntaxError(`Argument "typeDeterminerName" contains non-letter character(s)! (Advanced Determine - Batch Determine)`);
		};
		if (condition.length > 2) {
			throw new SyntaxError(`Argument "condition" is not match any pattern! (Advanced Determine - Batch Determine)`);
		};
		[typeDeterminerName, typeDeterminerOption] = condition;
	} else {
		throw new TypeError(`Argument "condition" must be type of string (non-nullable) or array! (Advanced Determine - Batch Determine)`);
	};
	if (items.length === 0) {
		throw new Error(`No input(s) at argument "items"! (Advanced Determine - Batch Determine)`);
	};
	let typeDeterminerFile = adCore.typeListMap[typeDeterminerName.toLowerCase()];
	if (typeof typeDeterminerFile !== "string") {
		throw new RangeError(`Argument "typeDeterminerName"'s value is not in the type list! (Advanced Determine - Batch Determine)`);
	};
	let typeDeterminerFunction;
	try {
		typeDeterminerFunction = require(`./is${typeDeterminerFile}.js`);
	} catch (error) {
		throw new Error(`Cannot find the module "is${typeDeterminerFile}.js"! Seems missing file(s). (Advanced Determine - Batch Determine)`);
	};
	let resultObject = {};
	Promise.allSettled(
