 ...
 * @returns {boolean}
 */
function areEqualObjectMeta(item1: object, item2: object): boolean {
	let item1ObjectMeta: ObjectMeta = new ObjectMeta(item1);
	let item2ObjectMeta: ObjectMeta = new ObjectMeta(item2);
	if (
		item1ObjectMeta.keysSymbol.length !== item2ObjectMeta.keysSymbol.length ||
	...
	for (let objectMetaType of ["keysSymbol", "entriesGetter", "entriesNonAccessor", "entriesSetter"]) {
		for (let item1ObjectMetaTypeEntry of item1ObjectMeta[objectMetaType]) {
			if (
				!item2ObjectMeta[objectMetaType].includes(item1ObjectMetaTypeEntry) ||
...
function areEqualInternal(item1: unknown, item2: unknown): boolean {
	...
	if (item1 instanceof Map && item2 instanceof Map) {
		...
		let item2Keys: unknown[] = Array.from(item2.keys());
		let item2Values: unknown[] = Array.from(item2.values());
		for (let [item1Key, item1Value] of item1.entries()) {
			let matchItem2KeysIndexes: number[] = [];
			item2Keys.forEach((item2Key: unknown, item2KeysIndex: number): void => {
				if (
			...
			if (matchItem2KeysIndexes.some((matchItem2KeysIndex: number): boolean => {
				let matchItem2Value: unknown = item2Values[matchItem2KeysIndex];
				return !(
 ...
 */
function areEqual(...items: unknown[]): boolean {
	if (items.length === 0) {
